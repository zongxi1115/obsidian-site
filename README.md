...
这套东西要吃的是任意一个人的笔记，所以原则是**渲染不出来就降级，别让构建挂掉**。
已经兜住的两类：

| 症状 | 原因 | 现在的行为 |
| --- | --- | --- |
| `Module not found: ./content/docs/xxx.md.js` | `remarkImage` 默认把 `![](相对路径)` 编译成真正的 `import`，图片没跟着提交就整个构建失败 | 关掉 `useImport`；同步时解析不到的图片直接降级成 alt 文本，并打一条 `[sync] 找不到图片 xxx` |
| `ShikiError: Language 'Xxx' not found` | 代码块写了 shiki 不认识的语言标记 | `fallbackLanguage: 'text'`，按纯文本渲染并警告 |

同步日志里的 `[sync] 找不到图片` / `[sync] 未解析的链接` 就是要修的地方 —— 构建不会挂，
但那些位置在站点上是空的。
